...
bool BMP::SetColor(int ColorNumber, RGBApixel NewColor) {
    ...
    if (BitDepth != 1 && BitDepth != 4 && BitDepth != 8) {
        if (EasyBMPwarnings) {
            cout << "EasyBMP Warning: Attempted to change color table for a "
                    "BMP object"
                 << endl
                 << "                 that lacks a color table. Ignoring "
                    "request."
                 << endl;
        }
        return false;
    ...
    if (!Colors) {
        if (EasyBMPwarnings) {
            cout << "EasyBMP Warning: Attempted to set a color, but the color "
                    "table"
                 << endl
...
RGBApixel BMP::GetColor(int ColorNumber) {
    ...
    if (BitDepth != 1 && BitDepth != 4 && BitDepth != 8) {
        if (EasyBMPwarnings) {
            cout << "EasyBMP Warning: Attempted to access color table for a "
                    "BMP object"
                 << endl
                 << "                 that lacks a color table. Ignoring "
                    "request."
...
bool BMP::SetBitDepth(int NewDepth) {
    ...
    if (NewDepth != 1 && NewDepth != 4 && NewDepth != 8 && NewDepth != 16 &&
        ...
        if (EasyBMPwarnings) {
            cout << "EasyBMP Warning: User attempted to set unsupported bit "
                    "depth "
                 << NewDepth << "." << endl
...
bool BMP::SetSize(int NewWidth, int NewHeight) {
    ...
    if (NewWidth <= 0 || NewHeight <= 0) {
        if (EasyBMPwarnings) {
            cout << "EasyBMP Warning: User attempted to set a non-positive "
                    "width or height."
                 << endl
                 << "                 Size remains unchanged at " << Width
                 << " x " << Height << "." << endl;
        }
        return false;
    }
...
bool BMP::WriteToFile(const char* FileName) {
    ...
    if (BitDepth != 16) {
        ...
        while (j > -1) {
            ...
            if (Success) {
                int BytesWritten =
                    (int)fwrite((char*)Buffer, 1, BufferSize, fp);
            ...
            if (!Success) {
                if (EasyBMPwarnings) {
                    cout << "EasyBMP Error: Could not write proper amount of "
                            "data."
                         << endl;
                }
                j = -1;
    ...
    if (BitDepth != 16) {
        ...
        while (j > -1) {
            ...
            if (BytesRead < BufferSize) {
                j = -1;
                if (EasyBMPwarnings) {
                    cout << "EasyBMP Error: Could not read proper amount of "
                            "data."
                         << endl;
                }
            } else {
                ...
                if (!Success) {
                    if (EasyBMPwarnings) {
                        cout << "EasyBMP Error: Could not read enough pixel "
                                "data!"
                             << endl;
                    }
                    j = -1;
...
bool BMP::CreateStandardColorTable(void) {
    ...
    if (BitDepth != 1 && BitDepth != 4 && BitDepth != 8) {
        if (EasyBMPwarnings) {
            cout
                << "EasyBMP Warning: Attempted to create color table at a bit"
                << endl
...
bool BMP::Write4bitRow(ebmpBYTE* Buffer, int BufferSize, int Row) {
    ...
    while (i < Width) {
        ...
        while (j < 2 && i < Width) {
            Index +=
                (PositionWeights[j] * (int)FindClosestColor(Pixels[i][Row]));
            i++;
            j++;
        }
        ...
        for (int i = 0; i < NewWidth - 1; i++) {
            ...
            InputImage(i, j)->Red =
                (ebmpBYTE)((1.0 - ThetaI - ThetaJ + ThetaI * ThetaJ) *
                               (OldImage(I, J)->Red) +
                           (ThetaI - ThetaI * ThetaJ) *
                               (OldImage(I + 1, J)->Red) +
                           (ThetaJ - ThetaI * ThetaJ) *
                               (OldImage(I, J + 1)->Red) +
            ...
            InputImage(i, j)->Green =
                (ebmpBYTE)((1.0 - ThetaI - ThetaJ + ThetaI * ThetaJ) *
                               OldImage(I, J)->Green +
                           (ThetaI - ThetaI * ThetaJ) *
                               OldImage(I + 1, J)->Green +
                           (ThetaJ - ThetaI * ThetaJ) *
                               OldImage(I, J + 1)->Green +
